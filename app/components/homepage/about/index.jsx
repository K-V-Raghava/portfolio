...
function AboutSection() {
  ...
  // LOGIC UPGRADE: Sort projects by date (Newest First) dynamically
  const sortedProjects = useMemo(() => {
    // Create a copy of the array so we don't mutate the original import
    return [...currentProjects].sort((a, b) => {
      // Convert strings to JS Date objects and subtract to sort descending
  ...
  const nextCard = () => {
    // Use the dynamically sorted array's length
        ...
        <div className="w-full min-h-[500px] lg:min-h-[550px] bg-[#1a1443]/60 backdrop-blur-md rounded-[1.5rem] p-5 lg:p-8 border border-white/5 shadow-lg flex flex-col">
          ...
          <div className="relative flex-grow flex items-center justify-center w-full">
            ...
            <div className="relative w-full h-[400px] lg:h-[450px] flex items-center justify-center overflow-hidden">
              ...
              {/* NOW MAPPING OVER sortedProjects INSTEAD OF currentProjects */}
              {sortedProjects.map((proj, index) => {
                ...
                return (
                  <div 
                    key={proj.id} // ID is now just for React keys, it no longer dictates order
                    onClick={() => {
                      if (isPrev) prevCard();
                      if (isNext) nextCard();
                    }}
                  ...
                  >
                    ...
                    <div className="flex flex-col flex-grow w-full">
                      ...
                      <div className="mb-5 flex flex-col lg:flex-row lg:items-center justify-between gap-3 border-b border-white/5 pb-3">
                        <h3 className="text-xl lg:text-2xl font-bold text-white">{proj.title}</h3>
                        <span className="text-xs text-emerald-400 font-mono bg-white/5 border border-emerald-400/20 px-4 py-1.5 rounded-full whitespace-nowrap">
                          {/* Will display the exact string you wrote in the data file */}
                          {proj.date}
                        </span>
                      </div>
            ...
            {/* Check against sortedProjects.length */}
